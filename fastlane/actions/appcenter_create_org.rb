module Fastlane
  module Actions
    class AppcenterCreateOrgAction < Action
      def self.description
        "Action to create an org in AppCenter"
      end

      def self.details
        "This action allows to create an organization in AppCenter and share with a list of emails."
      end

      def self.run(params)
        UI.message("Creating #{params[:owner_name]}")
        connection = self.connection
        url = "/v0.1/orgs"

        body ||= {
          "name": params[:owner_name],
          "display_name": params[:owner_display_name] || params[:distribution_name]
        }
...
") if ENV['DEBUG']
      ...
      case response.status
        when 200...300
          response.body
        when 409
          UI.error("Error, a distribution group with the same name already exists for this app")
          false
        when 401
      ...
      def self.available_options
        ...
        [
          ...
          FastlaneCore::ConfigItem.new(key: :owner_display_name,
                                       env_name: "FL_APPCENTER_CREATE_DISTRIBUTION_OWNER_DISPLAY_NAME", # The name of the environment variable
      ...
      def self.connection(upload_url = nil, dsym = false, csv = false)
        ...
        Faraday.new(options) do |builder|
          ...
          builder.response :json, content_type: /json$/ unless csv
